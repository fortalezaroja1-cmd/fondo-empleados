...
export default function AdminSocios({db,refresh}){
  ...
  return <div>
    ...
    lista.map(s=><div key={s.id} style={{background:'#fff',borderRadius:14,padding:'14px 16px',marginBottom:10,border:'1px solid rgba(0,0,0,0.08)'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:10,fontSize:13}}>
        <div style={{background:'#f5f4f0',borderRadius:8,padding:'8px 10px'}}>
          <div style={{color:'#6b6b66',fontSize:11}}>AHORRO ACUMULADO</div>
          <div style={{fontWeight:600}}>{fmt(s.ahorroAcumulado)}</div>
        </div>
        <div style={{background:'#f5f4f0',borderRadius:8,padding:'8px 10px'}}>
      ...
      <div style={{display:'flex',gap:8}}>
        <Btn onClick={()=>openEdit(s.id)} style={{flex:1,justifyContent:'center',minHeight:40,fontSize:14}}>Editar</Btn>
        <Btn danger onClick={()=>del(s.id)} style={{flex:1,justifyContent:'center',minHeight:40,fontSize:14}}>Eliminar</Btn>
      </div>
    </div>)}

    {modal&&<Modal title={modal.id?'Editar socio':'Nuevo socio'} onClose={()=>setModal(null)}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:14}}>
        ...
        <Field label="Utilidad % (vacío = global)"><input style={IS} type="number" step="0.01" placeholder={`${db.config.utilidadPct}%`} value={form.utilidadPct||''} onChange={e=>sf({utilidadPct:e.target.value})}/></Field>
        <Field label="Estado">
          <select style={IS} value={form.estado||'activo'} onChange={e=>sf({estado:e.target.value})}>
