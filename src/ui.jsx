...
export function Badge({ c, children }) {
  ...
  return <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 999, fontSize: 11, fontWeight: 500, whiteSpace: 'nowrap', ...(m[c] || m.gray) }}>{children}</span>
}
...
export function TW({ children }) { return <div style={{ overflowX: 'auto' }}>{children}</div> }
...
export function Spinner() {
  return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', flexDirection: 'column', gap: 12 }}>
    <div style={{ width: 32, height: 32, border: '3px solid var(--border)', borderTop: '3px solid var(--text)', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
    <p style={{ fontSize: 13, color: 'var(--text2)' }}>Cargando...</p>
