...
import AdminPeriodos from './admin/AdminPeriodos.jsx'
import AdminUtilidades from './admin/AdminUtilidades.jsx'
...
export default function AdminShell({db,refresh,perfil,onLogout}){
  ...
  return <div style={{display:'flex',flexDirection:'column',height:'100dvh',background:'#f5f4f0'}}>
    ...
    <div style={{flex:1,overflowY:'auto',padding:'16px'}}>
      ...
      {page==='utilidades'&&<AdminUtilidades db={db}/>}
      {page==='periodos'&&<AdminPeriodos db={db} refresh={refresh} esSuperAdmin={esSuperAdmin}/>}
    ...
    <div style={{background:'#fff',borderTop:'1px solid rgba(0,0,0,0.08)',display:'flex',flexShrink:0,paddingBottom:'env(safe-area-inset-bottom)',overflowX:'auto'}}>
      {PAGES.map(p=>{
        const active=page===p.id
        return <button key={p.id} onClick={()=>setPage(p.id)} style={{flex:'0 0 auto',display:'flex',flexDirection:'column',alignItems:'center',gap:3,padding:'10px 12px',border:'none',background:'transparent',cursor:'pointer',position:'relative',minWidth:0}}>
          <span style={{fontSize:18}}>{p.icon}</span>
          <span style={{fontSize:9,color:active?'#1a1a18':'#aaa',fontWeight:active?600:400,whiteSpace:'nowrap'}}>{p.label}</span>
          {p.id==='prestaya'&&pendPY>0&&<span style={{position:'absolute',top:6,right:4,background:'#854F0B',color:'#fff',borderRadius:999,fontSize:8,padding:'1px 4px',fontWeight:700}}>{pendPY}</span>}
